...
<div class="{{ notification_wrapper_classes }}" {% if not related_products %}style="display: none;"{% endif %}>
    <div class="{% if related_products %}col-md-6{% else %}notification position-relative {% if not add_to_cart_mobile %}col-12 float-right{% endif %}{% endif %}">
    ...
    {% if related_products %}
        ...
        <div class="col-md-6">
    {% endif %}
        <div class="row h5 font-weight-normal mt-2 mb-3{% if related_products %} mt-md-0{% endif %}">
            ...
            <strong class="js-cart-total col text-right">{{ cart.total | money | replace(',00','')}}</strong>
        </div>
        <a href="#" class="notification-button {% if related_products %}js-modal-close{% else %}js-cart-notification-close{% endif %} js-modal-open js-fullscreen-modal-open btn btn-primary btn-medium w-100 d-inline-block" data-toggle="#modal-cart" data-modal-url="modal-fullscreen-cart">
