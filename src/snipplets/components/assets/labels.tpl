...
{% set is_label_for_product_page = is_label_for_product_page | default(false) %}
	...
	{% if show_labels %}
		...
		{% else %}
			...
			{% if product.compare_at_price or product.promotional_offer %}
				<div class="{% if not product.promotional_offer and product %}js-offer-label{% endif %} label label-promotion" {% if (not product.compare_at_price and not product.promotional_offer) or not product.display_price %} style="display:none;" {% endif %} data-store="product-item-{% if product.compare_at_price %}offer{% else %}promotion{% endif %}-label">
			...
			{% if store_has_free_shipping %}
				<div class="{% if not product.free_shipping %}js-free-shipping-minimum-label {% endif %}label label-free-shipping" {% if not (product.free_shipping or product_price_above_free_shipping_minimum) %} style="display: none;" {% endif %}>{{ "ENVÍO GRATIS" | translate }}</div>
			{% endif %}
		{% endif %}
	{% endif %}
</div>

<div class="label-images{% if is_label_for_product_page %}-product{% endif %}">
	{% for cucarda in settings.cucardas %}
		{% for tag in product.tags %}
			{% set tag_split = tag | split("cucarda:") %}
			{% if tag_split.1 and tag_split.1 == cucarda.title %}
				<div class="label label-custom-image{% if is_label_for_product_page %}-product{% endif %}"><img alt="" src="{{ cucarda.image | static_url | settings_image_url('original') }}"/></div>
			{% endif %}
		{% endfor %}
	{% endfor %}
</div>
