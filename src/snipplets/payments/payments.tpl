                                ...
                                {% else %}
                                    ...
                                    {% for installment, data_installment in installments %}
                                        <div id="installment_{{ method }}_{{ installment }}">
                                            ...
                                            <strong class="js-installment-amount">{{ installment }}</strong> {% if store.country != 'BR' %}cuota{% if installment > 1 %}s{% endif %} de{% else %}x{% endif %} <strong class="js-installment-price">{{ (rounded_installment_value * 100) | money | replace(',00','')}}</strong>
                                ...
                                <div class="box">
                                    ...
                                    <h4 class="mb-1 font-weight-normal">
                                        ...
                                        {% if custom_payment.discount > 0 %}
                                            ...
                                            <span class="js-installments-one-payment price-compare">{{ product.price | money | replace(',00','')}}</span>
                                            <strong class="js-price-with-discount h3 text-brand" data-payment-discount="{{ custom_payment.discount }}">{{ price_with_discount | money | replace(',00','')}}</strong> 
                                        {% else %} 
                                            <strong class="js-installments-one-payment ml-3">{{ product.price | money | replace(',00','')}}</strong>
                                        {% endif %}
                                    </h4>
