        ...
        {% if store.country == 'AR' %}
            ...
            {% if installments_data['max_without_interests'] > 1 %}
                <h4 class="font-weight-normal mb-1">
                    ...
                    {{'de' | t}}
                    <strong class="js-modal-installment-price" data-installment="{{ installments_data['max_without_interests'] }}"> {{ (product.price / installments_data['max_without_interests']) | money | replace(',00','')}}</strong>
                </h4>
                <h6 class="font-weight-normal mb-2">
                    ...
                    <span class="mr-1">
                        <span>{{ 'Total: ' | translate }}</span><strong class="js-installments-one-payment">{{ product.price | money | replace(',00','')}}</strong>
                    </span>
                    <span class="mr-1">
                        <span>{{ 'En 1 pago: ' | translate }}</span><strong class="js-installments-one-payment">{{ product.price | money | replace(',00','')}}</strong>
                    </span>
                </h6>
            {% elseif installments_data['max_with_interests'] > 1 %}
                ...
                <h6 class="font-weight-normal mb-2">
                    <span>
                        <span>{{ 'O en 1 pago de: ' | translate }}</span><strong class="js-installments-one-payment">{{ product.price | money | replace(',00','')}}</strong>
                    </span>
                </h6>
            {% else %}
                <h4 class="font-weight-normal mb-1">
                    ...
                    {% for installment, data_installment in installments %}
                        ...
                        <tr id="installment_{{ method }}_{{ installment }}">
                            ...
                            <td>
                                ...
                                <strong><span class="js-installment-price">{{ (rounded_installment_value * 100) | money | replace(',00','')}}</span> </strong>
                            ...
                            <td class="js-installment-total-price text-right">
                                {{ total_value_in_cents | money | replace(',00','')}}
                            </td>
                        </tr>
                    {% endfor %}
                </tbody>
