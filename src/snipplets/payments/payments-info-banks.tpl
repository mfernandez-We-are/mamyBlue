      ...
      {% for installment, banks in gateways.cc.no_interest %}
        <div>
          ...
          <h4 class="font-weight-normal mb-1">
            ...
            {{'de' | t}}
            <strong class="js-modal-installment-price" data-installment="{{installment}}"> {{ (product.price / installment) | money | replace(',00','')}}</strong>
          </h4>
    ...
    {% if gateways.cc.interest is not null %}
      <div>
        ...
        <h6 class="font-weight-normal mb-2">
          ...
          <strong class="js-installments-one-payment">{{ product.price | money | replace(',00','')}}</strong>
        </h6>
...
{% if gateways.debit is not null or gateways.cash is not null or gateways.transfer is not null %}
  ...
  <div class="box">
    ...
    {% if gateways.debit is not null %}
      ...
      <h6 class="font-weight-normal mb-2">
        <span>{{ 'Precio:' | translate }} </span><strong class="js-installments-one-payment"> {{ product.price | money | replace(',00','')}}</strong>
      </h6>
    ...
    {% if gateways.transfer is not null %}
      ...
      <h6 class="font-weight-normal mb-2">
        <span>{{ 'Precio:' | translate }} </span><span class="js-installments-one-payment"> {{ product.price | money | replace(',00','')}}</span>
      </h6>
