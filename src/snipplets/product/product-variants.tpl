...
{% for tag in product.tags %}
    ...
    {% if tag_split.1 %}
        {% set size_guide_url = tag_split.1 %}
    {% endif %}
{% endfor %}
...
<div class="js-product-variants {% if quickshop %}js-product-quickshop-variants{% endif %} form-row {% if settings.bullet_variants %}mb-1{% else %}mb-3{% endif %}">
	...
	{% for variation in product.variations %}
		...
		<div class="js-product-variants-group {% if variation.name in ['Color', 'Cor'] %}js-color-variants-container mt-2{% endif %} {% if settings.bullet_variants or settings.image_color_variants %}col-12 {% if is_hidden_select or settings.bullet_variants %}mb-2{% endif %}{% endif %} {% if is_button_variant and show_size_guide and settings.size_guide_url and has_size_variations and loop.last %}mb-0{% endif %} {% if not is_button_variant %}{% if loop.length == 1 or loop.length == 3 or settings.image_color_variants %}col-md-8{% else %}col-6{% endif %}{% endif %}" data-variation-id="{{ variation.id }}">
			{% if quickshop %}
