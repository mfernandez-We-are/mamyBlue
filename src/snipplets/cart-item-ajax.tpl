  ...
  <div class="col-10 {% if cart_page %}col-md-11{% endif %}">
    ...
    <div class="{% if cart_page %}row align-items-center{% else %}w-100{% endif %}">
      ...
      <div class="cart-item-quantity {% if cart_page %}col-7 col-md-3{% endif %}" data-component="line-item.subtotal">
        {% embed "snipplets/components/forms/form-input.tpl" with{
          type_number: true, 
      ...
      {# Cart item subtotal #}
      <h6 class="js-cart-item-subtotal cart-item-subtotal {% if cart_page %}col-5 col-md-3 text-right text-md-center h4-md font-weight-bold{% endif %}" data-line-item-id="{{ item.id }}" data-component="subtotal.value" data-component-value={{ item.subtotal | money | replace(',00','')}}'>{{ item.subtotal | money | replace(',00','')}}</h6>
    </div>
  </div>
