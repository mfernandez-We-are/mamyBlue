...
<section class="account-page">
	<div class="container">
		...
		<div class="row justify-content-md-center">
			<div class="col-md-8">
				{% embed "snipplets/components/forms/form.tpl" with{form_id: 'login-form', submit_custom_class: 'btn-block', submit_text: 'Iniciar sesión' | translate, data_store: 'account-login' } %}

					{% block form_body %}

						{# Name input #}

						{% embed "snipplets/components/forms/form-input.tpl" with{input_for: 'email', type_email: true, input_value: result.email, input_name: 'email', input_custom_class: 'js-account-input', input_label_text: 'Email' | translate, input_required: true } %}
						{% endembed %}

						{# Password input #}

						{% embed "snipplets/components/forms/form-input.tpl" with{input_for: 'password', type_password: true, input_name: 'password', input_custom_class: 'js-account-input', input_help: true, input_help_link: store.customer_reset_password_url, input_link_class: 'btn-link-primary font-small mb-4 mt-3n', input_label_text: 'Contraseña' | translate, input_required: true } %}
